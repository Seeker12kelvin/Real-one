...
import { StyleCreatorContext } from '../../components/styleContext'
import { FaCheck } from 'react-icons/fa'
import "./createAccount.module.css"

const CreateAccount = () => {

 const {style, styleTwo, button, btnStyle, email, setEmail, emailWarning, inputValue, handleAccount} = useContext(StyleCreatorContext)
  ...
  return (
    <>
      ...
      <div className='text-black w-full mt-7 gap-4 flex flex-col items-start'>
        ...
        <form onSubmit={handleAccount} className='flex flex-col w-full'>
          <input name='Email' value={email} onChange={(e) => setEmail(e.target.value)} style={button && !inputValue ? styleTwo:style} type="text" placeholder='Your email' />
          {emailWarning}
          <button type='submit' className={btnStyle}>Get Started</button>
